...
/**
 * @brief Internal cogwheels for shion::when_any
 */
namespace when_any {
...
template <typename T>
struct arg_helper_s<shion::event_router_t<T>> {
	using type = event_router::awaitable<T>;

	template <typename U>
#ifndef _DOXYGEN
	requires (std::same_as<std::remove_cvref_t<U>, shion::event_router_t<T>>)
#endif
...
class when_any {
	...
	/**
	 * @brief Spawn a shion::job handling the Nth argument.
	 *
	 * @tparam N Index of the argument to handle
	 * @return shion::job Job handling the Nth argument
	 */
	template <size_t N>
	static shion::job make_job(std::shared_ptr<state_t> shared_state) {
		/**
	...
	/**
	 * @brief Spawn a shion::job to handle each awaitable.
...
public:
	...
	/**
	 * @brief On destruction the when_any will try to call @ref shion::task::cancel() cancel() on each of its awaitable if they have such a method.
	 *
	 * @note If you are looking to use a custom type with when_any and want it to cancel on its destruction,
	 * make sure it has a cancel() method, which will trigger an await_resume() throwing a shion::task_cancelled_exception.
