...
/* Internal cogwheels for shion::task */
namespace task {

/**
 * @brief A @ref shion::task "task"'s promise_t type, with special logic for handling nested tasks.
 *
...
/**
 * @brief The object automatically co_await-ed at the end of a @ref shion::task "task". Ensures nested coroutine chains are resolved, and the promise_t cleans up if it needs to.
 *
 * @tparam R Return type of the task
 */
template <typename R>
...
/**
 * @brief Alias for <a href="https://en.cppreference.com/w/cpp/coroutine/coroutine_handle"std::coroutine_handle</a> for a @ref shion::task "task"'s @ref promise_t.
 *
 * @tparam R Return type of the task
 */
template <typename R>
...
public:
	...
	/**
	 * @brief Cancel the task, it will stop the next time it uses co_await. On co_await-ing this task, throws shion::task_cancelled_exception.
	 *
	 * @return *this
	 */
...
struct promise_base : basic_promise<R> {
	...
	struct proxy_awaiter {
		 ...
		 *
		 * @throw shion::task_cancelled_exception If the task was cancelled
		 */
	 ...
	 *
	 * @throw shion::task_cancelled_exception On resumption if the task was cancelled
	 *
...
struct promise_t : promise_base<R> {
	 ...
	 * @brief Function called by the standard library when the coroutine is created.
	 *
	 * @return shion::task The coroutine object
	 */
	[[nodiscard]] shion::task<R> get_return_object() noexcept {
		return shion::task<R>{handle_t<R>::from_promise(*this)};
	}
...
struct promise_t<void> : promise_base<void> {
	 ...
	 */
	[[nodiscard]] shion::task<void> get_return_object() noexcept {
		return shion::task<void>{handle_t<void>::from_promise(*this)};
	}
...
std_coroutine::coroutine_handle<> final_awaiter<R>::await_suspend(handle_t<R> handle) const noexcept {
	...
	if (previous_state & state_flags::sf_awaited) { // co_await-ed, resume parent
		if (previous_state & state_flags::sf_broken) { // major bug, these should never be set together
			...
			std::cerr << "shion: task promise ended in both an awaited and dangling state. this is a bug and a memory leak, please report it to us!" << std::endl;
		}
